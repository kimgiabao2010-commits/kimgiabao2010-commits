...
/* ════════════════════════════════════════════════════════
   ...
   Kiểm định FastText model ngay sau khi retrain
   ════════════════════════════════════════════════════════ */
function ModelEvalPanel() {
  const [testText,    setTestText]    = useState('');
  const [evalResult,  setEvalResult]  = useState(null);  // {label, confidence, trust}
  const [evalStatus,  setEvalStatus]  = useState('idle'); // idle | loading | done | error
  const [healthInfo,  setHealthInfo]  = useState(null);   // {status, model_file}
  const [healthErr,   setHealthErr]   = useState(false);

  // Kiểm tra health FastText server khi component mount
  useEffect(() => {
    (async () => {
      try {
        const res  = await fetch('http://localhost:5001/health');
        if (res.ok) setHealthInfo(await res.json());
  ...
  const isScam     = evalResult?.prediction?.toLowerCase() === 'scam';
  const confPct    = evalResult?.confidence != null ? (evalResult.confidence * 100).toFixed(1) : null;
  const trustPct   = evalResult?.probability  != null ? (evalResult.probability  * 100).toFixed(1) : null;

  return (
    <div className="border-t border-gray-100 pt-6 flex flex-col gap-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h3 className="text-[0.65rem] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
          <CheckSquare size={12} strokeWidth={1.5} /> Model Evaluation — Kiểm định sau khi train
        </h3>
        {/* Health badge */}
        {healthInfo ? (
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 border border-emerald-200 rounded-sm text-[0.58rem] font-bold text-emerald-700 uppercase tracking-widest">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            FastText :5001 — ONLINE
          </span>
        ) : healthErr ? (
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-rose-50 border border-rose-200 rounded-sm text-[0.58rem] font-bold text-rose-600 uppercase tracking-widest">
            <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
            :5001 — OFFLINE
          </span>
        ) : (
          <span className="text-[0.58rem] text-gray-300 font-mono">Đang kiểm tra :5001...</span>
        )}
      </div>
      ...
      <div className="flex gap-2">
        <textarea
          ...
          onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) handleEval(); }}
          placeholder="Nhập đoạn text để kiểm định model... (Ctrl+Enter để gửi)"
          rows={2}
          className="flex-1 resize-none border border-gray-200 bg-gray-50 px-3 py-2.5 text-[0.7rem] font-mono text-gray-700 placeholder:text-gray-300 focus:outline-none focus:border-indigo-400 focus:bg-white transition-colors rounded"
          disabled={!!healthErr}
        />
        <button
          onClick={handleEval}
          disabled={evalStatus === 'loading' || !testText.trim() || !!healthErr}
          className={`px-4 py-2 text-[0.62rem] font-bold uppercase tracking-widest flex items-center gap-2 rounded transition-all ${
            evalStatus === 'loading' || !testText.trim() || healthErr
      ...
      {/* Result */}
      {evalStatus === 'done' && evalResult && !evalResult.error && (
        <div className={`rounded border p-4 flex flex-col gap-3 ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              {isScam
                ? <XCircle size={18} strokeWidth={1.5} className="text-rose-500" />
                : <CheckCircle2 size={18} strokeWidth={1.5} className="text-emerald-500" />
              }
              <span className={`text-sm font-black uppercase tracking-widest ${isScam ? 'text-rose-700' : 'text-emerald-700'}`}>
                {isScam ? '⚠️  SCAM DETECTED' : '✅  SAFE — LEGIT'}
              </span>
            </div>
            <span className="text-[0.6rem] font-mono font-bold text-gray-500 uppercase tracking-widest">FastText v2 (retrained)</span>
          </div>

          {/* Score bars */}
          <div className="grid grid-cols-2 gap-3">
            {confPct && (
              <div className="bg-white border border-gray-100 rounded p-3 flex flex-col gap-1.5">
                <span className="text-[0.55rem] font-bold text-gray-400 uppercase tracking-widest">Prediction Confidence</span>
                <span className={`text-xl font-black tracking-tight ${isScam ? 'text-rose-600' : 'text-emerald-600'}`}>{confPct}%</span>
                <div className="h-1 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${isScam ? 'bg-rose-500' : 'bg-emerald-500'}`}
                    style={{ width: `${confPct}%` }}
                  />
                </div>
                <span className="text-[0.55rem] text-gray-400">Model chắc chắn về phán quyết này</span>
              </div>
            )}
            {trustPct && (
              <div className="bg-white border border-gray-100 rounded p-3 flex flex-col gap-1.5">
                <span className="text-[0.55rem] font-bold text-gray-400 uppercase tracking-widest">Trust Score (Legit Prob.)</span>
                <span className="text-xl font-black tracking-tight text-indigo-600">{trustPct}%</span>
                <div className="h-1 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div
                ...
                <span className="text-[0.55rem] text-gray-400">Xác suất nội dung là hợp lệ</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded p-2.5">
            <span className="text-[0.55rem] font-bold text-gray-400 uppercase tracking-widest block mb-1">Input được phân tích</span>
            <p className="text-[0.65rem] font-mono text-gray-600 italic leading-relaxed">"{testText.length > 200 ? testText.substring(0, 200) + '...' : testText}"</p>
          </div>
        </div>
      )}
      ...
      {/* Error */}
      {evalStatus === 'error' && (
        <div className="p-3 bg-rose-50 border border-rose-200/60 rounded text-[0.65rem] font-mono text-rose-700 flex items-center gap-2">
          <XCircle size={12} strokeWidth={2} />
          Không kết nối được tới FastText server (Port 5001). Đảm bảo server đang chạy.
        </div>
      )}

      {/* Hint */}
      {evalStatus === 'idle' && (
        <p className="text-[0.6rem] text-gray-400 italic">
          💡 Dán thử một đoạn văn bản nghi ngờ hoặc an toàn để xác nhận model mới đã học đúng.
        </p>
      )}
    </div>
  );
